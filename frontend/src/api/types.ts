...
export interface PositionCheck {
  ...
  sts_pair_detected: boolean | null  // two tankers alongside = oil transfer
}
...
export interface Cluster {
  kind: 'anchorage' | 'activity'
  lat: number
  lon: number
  count: number       // anchorage: anchored ships · activity: distinct ships
  ...
  // anchorage only
  nearby?: number      // other flagged ships within the cluster radius
  ...
  // activity hotspot only
  event_count?: number // total anomalies in the cell (72h)
  variety?: number     // distinct pattern types
}
